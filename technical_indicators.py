"""
Technical Indicators v3.3
Fixes vs previous:
  - Division by zero guard in calculate_momentum_score (closes[-20] == 0)
  - Division by zero guard in calculate_relative_strength (bench[-period] == 0)
  - Removed unused import: Optional, Tuple
  - calculate_sma: period=0 guard added
"""
...
def detect_vcp(data: List[dict], min_contractions: int = 3) -> List[bool]:
    """VCP: week[0] = newest (bars i-4..i inclusive), week[3] = oldest."""
    result = [False] * len(data)
...
def detect_pocket_pivot(data: List[dict], lookback: int = 10) -> List[bool]:
    ...
    for i in range(lookback, len(data)):
        if data[i]["close"] <= data[i-1]["close"]:
            continue
        max_down = max(
            (data[j]["volume"] for j in range(i - lookback, i)
...
def detect_rs_divergence(stock: List[dict], bench: List[dict], lookback: int = 20) -> List[bool]:
    ...
    rs = [
        stock[i]["close"] / bench[i]["close"]
        if bench[i]["close"] > 0 else 1.0
        for i in range(n)
    ]
...
def validate_minervini_template(ohlcv: List[dict]) -> dict:
    """5-criteria SEPA check. Needs ≥100 bars; accuracy improves with 1y data."""
    if len(ohlcv) < 100:
        return {"passes": False, "reason": "Insufficient history (<100 bars)",
                "criteria": {}, "criteria_met": 0}
    ...
    high52        = max(d["high"] for d in window)
    low52         = min(d["low"]  for d in window)
    ...
    passes = all(c.values())
    return {
        "passes": passes, "criteria": c, "criteria_met": sum(c.values()),
        "pct_from_52w_high": pct_hi, "pct_from_52w_low": pct_lo,
        "sma50": s50, "sma150": s150, "sma200": s200,
        "reason": "All criteria met" if passes
                  else f"Failed: {', '.join(k for k, v in c.items() if not v)}",
    }


# ── RS Rank (IBD-style 1–99 percentile) ──────────────────────────────────────

def calculate_rs_rank(rs_scores: List[float]) -> List[int]:
    n = len(rs_scores)
    if n == 0: return []
    if n == 1: return [50]
    indexed = sorted(enumerate(rs_scores),
...
def calculate_correlation_matrix(sector_returns: Dict[str, List[float]], window: int = 30) -> Dict:
    ...
    for i, n1 in enumerate(names):
        for n2 in names[i:]:
            ...
            n  = min(len(r1), len(r2))
            if n < 5:
                result[(n1,n2)] = result[(n2,n1)] = 0.0; continue
            r1, r2 = r1[-n:], r2[-n:]
...
def count_new_highs_lows(stocks_data: List[dict]) -> dict:
    nh = sum(1 for s in stocks_data
             if s.get("high52w", 0) > 0
             and abs(s.get("price", 0) - s["high52w"]) / s["high52w"] < 0.02)
    nl = sum(1 for s in stocks_data
             if s.get("low52w", 0) > 0 and s.get("price", 0) > 0
...
def calculate_pct_above_ma(stocks_data: List[dict]) -> dict:
    ...
    return {
        "above_20dma":  sum(1 for s in stocks_data if s.get("above20dma"))  / n * 100,
...
def calculate_momentum_score(data: List[dict]) -> float:
    ...
    if len(closes) >= 20:
        base = closes[-20]
        if base and base != 0:                          # Bug fixed: guard /0
...
def calculate_relative_strength(stock: List[float], bench: List[float], period: int = 50) -> float:
    if len(stock) < period or len(bench) < period:
        return 0.0
    sp, bp = stock[-period], bench[-period]
    if sp == 0 or bp == 0:          # Bug fixed: guard /0
        return 0.0
    sr = (stock[-1] - sp) / sp
    br = (bench[-1] - bp) / bp
    return ((1 + sr) / (1 + br) - 1) * 100 if (1 + br) != 0 else 0.0


# ── RRG (JdK Method) ──────────────────────────────────────────────────────────

def calculate_rrg_values(stock: List[float], bench: List[float], period: int = 10) -> dict:
    """JdK RS-Ratio + RS-Momentum with forward-fill (not zero-fill) for NaN."""
    ...
    rr     = [rs[i]/rs_ma[i]*100 if not math.isnan(rs_ma[i]) and rs_ma[i]!=0
    ...
    return {"rs_ratio":    max(85.0, min(115.0, rs_ratio)),
