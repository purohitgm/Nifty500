...
# ===== NEW: EMA options for correlation =====
...
# ===== Helper: resample returns for timeframe =====
def resample_returns(df, timeframe):
"""Resample returns (already computed) for correlation analysis."""
resample_rules = {
"daily": None,
"weekly": 'W',
"monthly": 'M',
...
if rule is None:
return df.dropna()
return df.resample(rule).apply(lambda x: (1 + x).prod() - 1).dropna()

# ===== Helper: compute EMA-based returns =====
def ema_returns(price_df: pd.DataFrame, period: int) -> pd.DataFrame:
"""
Compute log returns of EMA(period) for each column.
This is what we will correlate.
"""
...
st.subheader(f"🔗 {corr_tf.replace('_', ' ').title()} Correlation Matrix ({basis_label})")

# 1. Compute base returns according to basis
...
# 2. Apply timeframe resampling to those returns
...
title=f"{corr_tf.replace('_', ' ').title()} Correlation Heatmap ({basis_label})",
...
# Correlation basis returns
if 'base_returns' in locals() and not base_returns.empty:
csv_corr_basis = base_returns.to_csv()
st.download_button(
f"📐 {basis_label} Returns (CSV)",
csv_corr_basis,
f"{basis_label.replace(' ', '_').lower()}_returns.csv",
"text/csv"
)
